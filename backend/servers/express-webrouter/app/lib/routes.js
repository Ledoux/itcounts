...
exports.default = function (app) {
  ...
  app.get('/api/dataviz_bubble_V2', function (req, res) {
    _models.Deputes.aggregate([{ $unwind: "$groupes_parlementaires" }, {
      $group: {
  ...
  app.get('/api/hemicycle/form', function (req, res) {
    _models.Deputes.aggregate([{ "$group": { "_id": { sigle: "$groupe_sigle", groupe: "$groupe.organisme" } } }, { "$sort": { "_id.groupe": 1 } }], function (err, groupes) {
      if (err) {
        res.send(err);
      }
      _models.Deputes.aggregate([{ $group: { _id: 1, max: { $max: "$nb_mandats" },
          min: { $min: "$nb_mandats" } } }], function (err, nb_mandats) {
        if (err) {
          res.json(err);
        }
        _models.Deputes.aggregate([{ $project: { "ageInMillis": { $subtract: [new Date(), "$date_naissance"] } } }, { $project: { "age": { $divide: ["$ageInMillis", 31558464000] } } }, { $project: { "age": { $subtract: ["$age", { $mod: ["$age", 1] }] } } }, { $group: { _id: 1, max: { $max: "$age" }, min: { $min: "$age" } } }], function (err, dates_naissance) {
          if (err) {
            res.json(err);
          }
          _models.Deputes.aggregate([{ "$group": { "_id": { num: "$region.num", nom: "$region.nom" } } }, { "$sort": { "_id.nom": 1 } }], function (err, region) {
            if (err) {
              res.json(err);
            }
  ...
  app.get('/api/hemicycle/search', function (req, res) {
    ...
    _models.Deputes.find(query, { nom: 1, sexe: 1, place_en_hemicycle: 1, url_an: 1 }, function (err, cursor) {
      if (err) {
        res.json(err);
      }
      res.json(cursor);
